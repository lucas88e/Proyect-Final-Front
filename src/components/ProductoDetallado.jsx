...
function ProductoDetallado() {
  ...
  const handlePujaClick = (product) => {
    ...
    localStorage.setItem("puja", product.newPrice)
  };

  const fechaFormateada = product.fechaFinal ? format(new Date(product.fechaFinal),'dd-MM-yyyy') : '' // Formateo de fecha horas y minutos
    ...
    <Card key={product._id} style={{ width: '25rem' }}>
        ...
        <Card.Body>
          ...
          <Card.Text>
            ...
            </Card.Text> <span>{product.descripcion}</span>
          
          <Card.Text>
            <b>Categoria:</b> <span>{product.categoria}</span>
          </Card.Text>
          <Card.Text>
            
            <b>Talla:</b> <span>{product.talla}</span>
          </Card.Text>
          <Card.Text>
            <b>La puja Finaliza el dia:</b> <span className='tamaño'>{fechaFormateada}</span>
          </Card.Text>
          <Card.Text>
            <b>Puja de salida:</b> <span className='tamaño'>{product.precio}€</span>
          </Card.Text>
          <Card.Text>
            <b>Puja Actual:</b> <span  className={pujaConfirmada ? "puja-confirmada tamaño" : 'tamaño'}>{product.newPrice}€</span>
          </Card.Text>
          <input
            type='number'
            className='input'
